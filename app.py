...
# -------------------------------------------------
# CORE AI FUNCTION (memory + restriction)
# -------------------------------------------------
def ask_syntra(user_text: str, restaurant_key: str) -> str:
    ...
    prompt = f"""
You are Syntra AI, a professional restaurant assistant for '{restaurant_key}'.

Here is verified information about the restaurant:
...
Rules:
- ONLY answer questions about {restaurant_key}, its menu, prices, hours, offers, delivery, etc.
- If question is unrelated (e.g. science, jokes, politics), say:
...
# 🗑 CLEAR CHAT ENDPOINT
# -------------------------------------------------
@app.delete("/clear/{restaurant_name}")
async def clear_chat_history(restaurant_name: str):
    """Delete all chat messages for a restaurant"""
    try:
        collection = db[restaurant_name]
        ...
        return JSONResponse({
            ...
            "message": f"Cleared chat history for {restaurant_name}."
        })
    except Exception as e:
